...
class BaseMilstein(base_solver.BaseSDESolver, metaclass=abc.ABCMeta):
    ...
    def step(self, t0, t1, y0, aux, extra0):
        ...
        if self.options[METHOD_OPTIONS.grad_free]:
            f, g = self.sde.f_and_g(t0, y0, aux)
            ...
            g_prime = self.sde.g(t0, y0_prime, aux)
            g_prod_I_k = self.sde.prod(g, I_k)
            gdg_prod = self.sde.prod(g_prime - g, v) / (2 * sqrt_dt)
        else:
            f = self.sde.f(t0, y0, aux)
            g_prod_I_k, gdg_prod = self.sde.g_prod_and_gdg_prod(t0, y0, aux, I_k, 0.5 * v)
